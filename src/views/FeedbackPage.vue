        ...
        <div class="container">
          ...
          <div class="glass-card submitted-card" v-if="alreadySubmitted">
            ...
            <div class="existing-review">
              <div class="star-display">
                <span v-for="s in 5" :key="s" class="star-display-item"
                  :class="s <= existingScore ? 'star-filled' : 'star-empty'">★</span>
              </div>
          ...
          <div v-else>
            ...
            <div class="glass-card rating-card">
              ...
              <div class="stars-row">
                <div v-for="s in 5" :key="s" class="star" :class="s <= hoverStar || s <= score ? 'star-active' : ''"
                  @click="score = s" @mouseenter="hoverStar = s" @mouseleave="hoverStar = 0">★</div>
              </div>
            ...
            <div class="glass-card">
              ...
              <textarea v-model="comments" :placeholder="commentPlaceholder" class="comment-input"
            ...
            <div class="glass-card">
              ...
              <div class="tags-row">
                <div v-for="tag in quickTags" :key="tag"
                  :class="['tag', selectedTags.includes(tag) ? 'tag-active' : '']" @click="toggleTag(tag)">{{ tag }}
                </div>
              </div>
            </div>
            ...
            <!-- Submit -->
            <button class="btn-submit" @click="submitFeedback" :disabled="isLoading || score === 0">
...
const scoreLabel = computed(() => {
  ...
  return hoverStar.value > 0 ? labels[hoverStar.value] : labels[score.value] || 'Tap a star to rate'
})
...
const submitFeedback = async () => {
  ...
  try {
    ...
    if (!existing.includes(transactionId.value)) {
      existing.push(transactionId.value)
      localStorage.setItem('submittedFeedbacks', JSON.stringify(existing))
    }
